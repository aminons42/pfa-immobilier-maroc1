...
def predict_price(
    ...
    central_log = float(
        xgb.predict(vector)[0]
    )

    # RETOUR AU PRIX RÉEL
    central = float(
        np.expm1(central_log)
    )
    ...
    tree_preds_log = np.stack([
        estimator.predict(vector)[0]
        for estimator in rf.estimators_
    ])

    tree_preds_real = np.expm1(
        tree_preds_log
    )

    std = float(
        np.std(tree_preds_real)
    )
    ...
    city_row = (
        db.query(City)
        .filter(City.name == payload.ville)
        .first()
    )
    ...
    avg_city = (
        city_row.avg_prix_m2
        if city_row
        else 0.0
    )

    avg_quartier = (
        quartier_row.avg_prix_m2
        if quartier_row
        else avg_city
    )
    ...
    position = (
        "above_market"
        if prix_m2_estime >= avg_quartier
        else "below_market"
    )
    ...
    return {
        ...
        "market_context": {
            "avg_prix_m2_city": round(
                avg_city,
                2,
            ),

            "avg_prix_m2_quartier": round(
                avg_quartier,
                2,
            ),
